...
(text-mode is used here, no additional setup required)
...
- select whole content, then activate secondary selection with ~G~
- ~b~ for back-word to create fake cursors at each word beginning
...
- select whole content, then activate secondary selection with ~G~
- ~- f -~ to backward search for character ~-~, will create fake cursor at each ~-~
- quick start recording and switch to insert state with ~c~ (current char is deleted)
- type ~_~
...
- select whole content, then activate secondary selection with ~G~
- ~x~ for line to create fake regions at each line
- ~F3~ to start macro recording(default fake cursors are on the same column)
- select bar ~w~ for mark-word, then activate secondary selection with ~G~
- select foo, swap with secondary selection with ~R~ for swap-grab.
- ~F4~ to end macro recording and apply to all fake cursors
...
- select whole content, then activate secondary selection with ~G~
- move to bar, select it with ~w~ for mark-word, create fake regions at each bar
- quick start recording and switch to insert state with ~c~ (current bar is deleted)
- type baz
