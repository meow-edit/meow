...
This can be changed by add following code to ~meow-setup~ (before ~meow-leader-define-key~).

#+begin_src emacs-lisp
  (add-to-list 'meow-keymap-alist (cons 'leader <a keymap as leader keymap>))
#+end_src
